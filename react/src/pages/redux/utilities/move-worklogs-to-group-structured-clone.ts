 ...
 * Uses `structuredClone` to create a draft state from the existing one, which allows mutating it directly.
...
const moveWorklogsToGroup = (
	...
	return (
		...
		const stateNextDraft = structuredClone(getState().page);
...
export {
	moveWorklogsToGroup as moveWorklogsToGroupStructuredClone,
};
